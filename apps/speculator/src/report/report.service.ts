...
import { Ticker } from 'src/tickers/interfaces';
import { Exchange, TickerType, MostActives, Movers, NetBuySellList, Index } from 'src/tickers/enums';
import { getMoneyFlowFromTickersByDate, getMostActivesFromTickers, getMoversFromTickers, getNetBuySellListFromTickers } from 'src/tickers/utils';
...
export class ReportService {
  ...
  async addMarketInfoSheet(workbook: ExcelJS.Workbook, options: ExportOptions) {
    ...
    const data = await this.tickersService.getMarketInfo({ date, days });
  ...
  async addTwseMoneyFlowSheet(workbook: ExcelJS.Workbook, options: ExportOptions) {
    const { date, days } = options;
    const tickersByDate = await this.tickersService.getTickersByDate({ date, type: TickerType.Index, exchange: Exchange.TWSE, days });
  ...
  async addTpexMoneyFlowSheet(workbook: ExcelJS.Workbook, options: ExportOptions) {
    const { date, days } = options;
    const tickersByDate = await this.tickersService.getTickersByDate({ date, type: TickerType.Index, exchange: Exchange.TPEx, days });
  ...
  async addTwseMostActivesSheet(workbook: ExcelJS.Workbook, options: ExportOptions) {
    ...
    const data = await this.tickersService.getTickersByDate({ date, exchange: Exchange.TWSE, type: TickerType.Equity, days: 5 });
  ...
  async addTpexMostActivesSheet(workbook: ExcelJS.Workbook, options: ExportOptions) {
    ...
    const data = await this.tickersService.getTickersByDate({ date, exchange: Exchange.TPEx, type: TickerType.Equity, days: 5 });
